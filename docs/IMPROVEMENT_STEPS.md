...
- Micro 发现使用独立 03 Prompt 和临时线程，先强制检索至少两份独立攻略并形成观光短名单，再按攻略共识、地标性、拍照价值、主题匹配度和可访问性筛选。
- 服务端按 Macro 建议停留时长确定 3/5/7/9 个可靠兴趣点下限并扣除已有可靠点；AI 只研究、筛选和排序，不能降低目标。每次独立研究一个 Macro、最多 9 个 Candidate，并按当前顺序逐目的地保存。
- 03 的每次初始研究和补位各自使用 5 分钟超时；等待期间每 30 秒显示安全心跳。单个目的地失败后继续处理后续目的地，最终保留已保存结果并将存在缺口的总任务标记为失败。
- 服务端先校验临时显著性、体验类型、可访问点类型和攻略研究依据，并排除交通、住宿、餐饮、停车、行政、普通游客服务设施及泛称地理实体，再执行地图预检、行政区冲突检查和一次缺口补位；地图结果不作为热门程度依据。
- 抽象区域、活动概念和整条线路只有在攻略明确推荐时才能具体化为正式景点入口、观景点、场馆或体验报到点，不得落到游客中心或其他功能设施。
