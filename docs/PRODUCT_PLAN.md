...
- Micro Discovery 是观光候选池。自动推荐仅接受景点、地标、经典拍照点、观景台、博物馆、文化场馆、自然景观和正式体验入口；交通、住宿、餐饮、停车、行政和普通游客服务设施只能作为其他流程中的功能节点，不能填充兴趣点数量；
- AI 必须先实时检索多份相互独立的旅游攻略、目的地榜单或编辑推荐，再按攻略共识、地标性、拍照价值、旅行主题匹配度和当前可访问性筛选；官方网站用于核验名称和状态，不单独代表观光热度；
- 每个 Micro Candidate 必须提供临时 `prominence`、`experienceTypes`、`visitPointType` 和 `researchBasis` 供服务端校验，其中 `researchBasis` 必须包含 `multi_guide_consensus`；这些字段不写入 canonical plan、旅行数据库、日志或前端接口，也不保存来源链接；
- 服务端根据 Macro 建议停留时长确定可靠兴趣点总量下限：1 天 3 个、2 天 5 个、3 天 7 个、4 天及以上 9 个；时长未知按 1 天处理，并扣除未排除且已可靠定位的具体观光点；AI 不得降低固定目标；
- 每次只为一个 Macro 启动独立临时研究线程，Candidate 最多 9 个；多个目的地按当前顺序串行处理并逐项保存，不静默截断后续目的地；
- 每次初始研究或补位各自最多等待 5 分钟，并每 30 秒显示不含来源链接的安全心跳；单个目的地失败后继续处理后续目的地，全部结束后只要仍有缺口就保留成功结果并将总任务标记为失败；
- 每个 Candidate 必须对应一个当前仍存在、可由公开地图服务唯一识别的实体；
- “葡萄酒产区”“装饰艺术街区”“整条步道”“观鲸体验”等区域或活动概念，只有在攻略将某个正式景点入口、观景点、场馆或体验报到点本身作为观光目的地推荐时，才能具体化为该单点；
- 不得为了具体化泛称而臆造或推荐游客中心、信息中心，也不得用机场、车站、港口、酒店或餐厅替代真正观光点；
- 优先使用当前官方名称、本地名称或英文名称；怀疑闭馆、改名或搬迁时，由生成 Agent 先核验官方网站；
- AI 草稿先经过观光语义门槛，再经过非写入式地图预检；地图 Provider 只用于排除错误设施类型、行政区冲突并确认可定位性，不能用于判断热门程度。无法通过的候选直接舍弃并触发同范围补位；补位后仍未达到下限时保存部分结果和摘要，但任务标记为失败。
...
↓
按 Macro 对比服务端 3/5/7/9 固定下限；存在缺口时仅对缺口执行一次 scoped 自动补位
↓
地图 Provider 搜索
↓
...
- 用户主动生成兴趣点时，地图预检淘汰造成的目标缺口立即自动补位，不延迟到“生成行程”；
...
第 4 步“概览”和第 5 步“行程”都支持：

- 全览：同时显示当前阶段全部 Day 的路线和节点；
- 单日：只显示选中 Day 的路线和节点；
- 每个 Day 使用稳定且互不相同的路线颜色，并在地图图例中明确对应关系；
- 鼠标悬停每个 Route Leg 时显示 Day、实际交通方式、Provider 距离和 Provider 时长。

Macro 概览只展示 Macro 跨目的地路线，详细行程只展示逐日详细路线，两者不得混用。距离、时长和 geometry 只能来自 Route Provider；缺失时显示待计算，不由 AI、前端速度估算或直线距离补造。dirty 路线必须标明为旧路线并保持弱化。
