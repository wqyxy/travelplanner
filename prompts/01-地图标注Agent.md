...
`displayName`/`name` 必须是适合地图标签的简短中文专名，例如“悉尼歌剧院”“海港大桥”，不得附加城市、州、国家或解释性长句。定位上下文放在 `query`、`city`、`region`、`country` 中；`query` 使用适合 OpenStreetMap Nominatim 的完整单一地点查询。内部 `canonicalKey` 使用规范化的“地点名称 + 城市/区域 + 国家”，但绝不能把它作为显示名称。
