...
import type { AiTask, AppSettings, CandidatePreference, PlanCommand, ProposalScope, ProviderPlaceCandidate, Trip, Workspace, WorkspaceSelection } from "./v2-types";
...
export default function App() {
  ...
  const selectedCandidateId = selection.type === "candidate" ? selection.id : null;
  const selectedDayId = selection.type === "day" ? selection.id : null;
  ...
  const loadTrip = async (id: string, resetSelection = true) => {
    ...
    try {
      ...
      if (resetSelection) { setSelection(next.trip.plan.days.length ? { type: "candidate_pool", id: null } : { type: "candidate_pool", id: null }); setTab(next.trip.plan.days.length ? "itinerary" : "places"); }
  ...
  const runAction = async <T,>(operation: () => Promise<T>, fallback: string) => {
    ...
    try { return await operation(); } catch (cause) { setError(cause instanceof Error ? cause.message : fallback); return undefined; } finally { setBusy(false); }
  };
    ...
    const connect = () => {
      ...
      socket.onmessage = (event) => { try {
        ...
        if (item.kind === "ai-task.updated") {
          ...
          if (!["starting", "running", "waiting", "reconnecting"].includes(String(item.payload?.status))) void refreshWorkspace();
          return;
        }
        if (["travel.document.changed", "travel.resolution.changed", "travel.route.changed", "travel.proposal.changed", "travel.turn.changed"].includes(item.kind)) {
          void refreshWorkspace(); void refreshTrips(false);
        }
  ...
  const createTrip = async () => { await runAction(async () => { const result = await api<{ trip: Trip }>("/api/trips", { method: "POST", body: "{}" }); setTrash(false); await refreshTrips(false); await loadTrip(result.trip.id); }, "无法新建旅行。"); };
  const send = async (message: string) => { if (!trip) return; await runAction(async () => { await api(`/api/trips/${trip.id}/turns`, { method: "POST", body: JSON.stringify({ message }) }); await refreshWorkspace(); }, "无法发送旅行需求。"); };
  const createProposal = async (message: string, scope: ProposalScope) => { if (!trip) return; await runAction(async () => { await api(proposalCreatePath(trip.id), { method: "POST", body: JSON.stringify(proposalCreateBody(message, scope)) }); await refreshWorkspace(); }, "无法生成 AI 修改建议。"); };
  const proposalAction = async (proposalId: string, action: ProposalAction) => { if (!trip) return; await runAction(async () => { await api(proposalActionPath(trip.id, proposalId, action), { method: "POST", body: "{}" }); await loadTrip(trip.id, false); if (action !== "reject") await refreshTrips(false); }, action === "apply" ? "无法应用 AI 修改建议。" : action === "undo" ? "无法撤销 AI 修改建议。" : "无法取消 AI 修改建议。"); };
  ...
  const stopLatestTask = async () => { const active = tasks.find((task) => ACTIVE_TASKS.has(task.status)); if (active) await stopTask(active.id); };
  const discover = async () => { if (!trip) return; await runAction(async () => { await api(`/api/trips/${trip.id}/candidates/discover`, { method: "POST", body: "{}" }); await refreshWorkspace(); }, "无法生成地点推荐。"); };
  const setPreference = async (candidateIds: string[], preference: CandidatePreference) => { if (!trip || !candidateIds.length) return; await runAction(async () => { if (candidateIds.length === 1) await api(`/api/trips/${trip.id}/candidates/${encodeURIComponent(candidateIds[0])}`, { method: "PATCH", body: JSON.stringify({ expectedGeneration: trip.contentGeneration, preference }) }); else await api(`/api/trips/${trip.id}/candidates/batch`, { method: "POST", body: JSON.stringify({ expectedGeneration: trip.contentGeneration, candidateIds, preference }) }); await loadTrip(trip.id, false); await refreshTrips(false); }, "无法修改地点优先级。"); };
  ...
  const searchResolutionCandidates = async (placeId: string) => { if (!trip) return []; return (await api<{ candidates: ProviderPlaceCandidate[] }>(`/api/trips/${trip.id}/resolutions/${encodeURIComponent(placeId)}/candidates?expectedGeneration=${trip.contentGeneration}`)).candidates; };
  ...
  const generatePlan = async () => { if (!trip) return; await runAction(async () => { await api(`/api/trips/${trip.id}/plan/generate`, { method: "POST", body: "{}" }); setTab("itinerary"); await refreshWorkspace(); }, "无法生成按天行程。"); };
  ...
  const manage = async (item: Trip, action: "rename" | "duplicate" | "trash" | "restore" | "permanent") => { await runAction(async () => {
    ...
    if (action === "trash") { if (!window.confirm(`将“${item.title}”移入回收站？`)) return; await api(`/api/trips/${item.id}`, { method: "DELETE" }); if (trip?.id === item.id) { loadToken.current += 1; setWorkspace(null); setSelection({ type: "candidate_pool", id: null }); } await refreshTrips(false); }
