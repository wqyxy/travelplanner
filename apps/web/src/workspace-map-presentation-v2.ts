...
export function itineraryPointFeatures(workspace: Workspace, selectedDayId: string | null): WorkspaceMapPointFeature[] {
  ...
  for (const day of days) {
    ...
    add({ id: day.startAnchor.id, entityType: "anchor", placeId: day.startAnchor.placeId, mark: "起", nameFallback: day.startAnchor.label || "出发地点" });
    ...
    add({ id: day.endAnchor.id, entityType: "anchor", placeId: day.endAnchor.placeId, mark: "终", nameFallback: day.endAnchor.label || "结束地点" });
  }
  return features;
}
    ...
    return state.route?.legs.flatMap((leg) => {
      ...
      return [{
        ...
        properties: {
          ...
          status: leg.status,
          dirty: state.dirty,
          distanceKm: state.dirty ? null : leg.distanceKm,
          durationMinutes: state.dirty ? null : leg.durationMinutes,
          warning: leg.warning || "",
