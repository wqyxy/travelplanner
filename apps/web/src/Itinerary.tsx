...
import type { Itinerary as ItineraryData, ItineraryLanguage, Place, TransportMode, Verification } from "./types";
...
const verificationLabels = { verified: "已核验", estimated: "估算", unverified: "待核验" } as const;
const name = (place: Place | undefined, fallback: string, language: ItineraryLanguage) => placeNameLines(place, fallback, language).join(" · ");
const verification = (value: Verification | null, label?: string) => value ? <small className={`verification ${value.status}`} title={value.checkedAt ? `核验时间：${value.checkedAt}` : undefined}>{label ? `${label}：` : ""}{verificationLabels[value.status]}</small> : null;
const list = (values: string[]) => values.length ? values.join("、") : null;

export function Itinerary({ itinerary, language, selection, onSelectAll, onSelectDay, onLanguageChange, fullscreen, onToggleFullscreen }: {
    ...
    {itinerary.stage === "planning" ? <div className="planning-summary"><p>通过 AI Chat 补充旅行信息；信息足够后，AI 会先征求确认，再生成完整初稿。</p>{planningFacts.length > 0 && <dl>{planningFacts.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>}{itinerary.trip.assumptions.length > 0 && <div className="assumptions"><strong>透明假设</strong>{itinerary.trip.assumptions.map((item, index) => <p key={`${item.text}:${index}`}>{item.text} <small>{item.source} · {item.confidence}</small></p>)}</div>}</div> : itinerary.days.map((day) => <article className={`day-card ${selection.scope === "day" && selection.dayNumber === day.dayNumber ? "selected" : ""}`} key={day.id} onClick={() => onSelectDay(day.dayNumber)} tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelectDay(day.dayNumber); } }}>
      ...
      {day.stops.map((stop) => {
        ...
        return <div className="activity" key={stop.id}>
          ...
          <div>{stop.transportFromPrevious && <div className="transport-line"><span>{modeLabels[stop.transportFromPrevious.mode]}{stop.transportFromPrevious.durationMinutes !== null ? ` · ${stop.transportFromPrevious.durationMinutes} 分钟` : ""}</span>{verification(stop.transportFromPrevious.verification)}{stop.transportFromPrevious.note && <small>{stop.transportFromPrevious.note}</small>}</div>}<strong>{name(places.get(stop.placeId), stop.activity, language)}</strong><p>{stop.activity}{stop.durationMinutes !== null ? ` · 停留 ${stop.durationMinutes} 分钟` : ""}</p><div className="verification-row">{verification(stop.scheduleVerification, "日程")}{verification(stop.costVerification, "费用")}</div>{stop.costNote && <small className="stop-note">费用：{stop.costNote}</small>}{stop.notes && <small className="stop-note">{stop.notes}</small>}</div>
        </div>;
      })}
    </article>)}
