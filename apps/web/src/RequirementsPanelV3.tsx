...
const fields: Array<{ key: BriefField; label: string; placeholder: string; required?: boolean }> = [
...
export function RequirementsPanelV3({ facts, busy, onSave, onGenerate }: {
  ...
  return <section className="workspace-requirements-v3 requirements-form-v3">
    <div><p className="eyebrow">START HERE</p><h2>旅行需求</h2><p>填写你已知的信息即可，可以用“英国”“9 月”“10 天左右”这类自然表达。离开输入框会自动保存；旅行需求 AI 也会维护同一份内容。</p></div>
    <div className="requirements-fields-v3">
      {fields.map((field) => <label key={field.key}><span>{field.label}{field.required ? <em>必填</em> : null}</span><input value={draft[field.key]} disabled={busy} placeholder={field.placeholder} onChange={(event) => { setStatus("idle"); setDraft((current) => ({ ...current, [field.key]: event.target.value })); }} onBlur={() => void save(field.key)}/></label>)}
    </div>
    ...
    <button className="button primary workspace-primary-cta-v3" type="button" disabled={busy || !destinationReady} title={destinationReady ? undefined : "请先填写并保存目的地"} onClick={() => void onGenerate()}><Sparkles size={15}/>生成目的地建议</button>
  </section>;
}
