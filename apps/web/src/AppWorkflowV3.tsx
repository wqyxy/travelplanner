...
export default function AppWorkflowV3() {
  ...
  const updatePlace = async (placeId: string, changes: WorkflowPlaceEditChangesV3) => {
    ...
    return (await runAction(async () => { await api(`/api/trips/${trip.id}/commands`, { method: "POST", body: JSON.stringify({ expectedGeneration: trip.contentGeneration, commands: [{ type: "update_place", placeId, changes }] }) }); await loadTrip(trip.id, false); await refreshTrips(false); return true; }, "无法保存地点名称。")) === true;
  };
  ...
  const applyGoogleMapsLink = async (placeId: string, url: string, changes: WorkflowPlaceEditChangesV3) => {
    if (!trip) return false;
    return (await runAction(async () => { await api(`/api/trips/${trip.id}/places/${encodeURIComponent(placeId)}/google-maps`, { method: "PUT", body: JSON.stringify({ expectedGeneration: trip.contentGeneration, url, changes }) }); await loadTrip(trip.id, false); await refreshTrips(false); return true; }, "无法通过 Google Maps 链接保存地点。")) === true;
  };
  ...
  const retryResolutions = async (placeIds: string[], force = false) => {
    ...
    return (await runAction(async () => { await api(`/api/trips/${trip.id}/resolutions/retry`, { method: "POST", body: JSON.stringify({ expectedGeneration: trip.contentGeneration, placeIds, force }) }); await loadTrip(trip.id, false); return true; }, force ? "无法重新定位地点。" : "无法重新识别地点。")) === true;
  };
      ...
      <div className="main-workspace main-workspace-v3"><div className={`travel-workspace travel-workspace-v3 ${focus ? `focus-${focus}` : ""}`} ref={workspaceElement} style={{ gridTemplateColumns: `${settings.ui.workspaceSplitRatio}fr 10px ${1 - settings.ui.workspaceSplitRatio}fr` }}>
        ...
        <div className="workspace-side-v3">{workspace ? <>
          ...
          <div className="workspace-step-content-v3">
            ...
            : step === "backbone" ? <CandidateWorkflowPanelV3 mode="backbone" workspace={legacyWorkspace} selectedCandidateId={selectedCandidateId} busy={working} macroNeedsUpdate={Boolean(macroNeedsUpdate)} onSelectCandidate={(candidateId) => setSelection({ type: "candidate", id: candidateId })} onSetPreference={setPreference} onDiscover={() => startCta("destinations", "destination.generate")} onAddCandidate={addCandidate} onUpdatePlace={updatePlace} onPreviewGoogleMapsLink={previewGoogleMapsLink} onApplyGoogleMapsLink={applyGoogleMapsLink} onRemoveCandidate={removeCandidate} onContinue={async () => gotoStep("skeleton")} onGoToSkeleton={() => gotoStep("skeleton")} onRetry={retryResolutions} onBeginMapPick={(placeId) => { setMapPickPlaceId(placeId); setFocus("map"); }}/>
            ...
            : step === "interests" ? <CandidateWorkflowPanelV3 mode="interests" workspace={legacyWorkspace} selectedCandidateId={selectedCandidateId} busy={working} macroNeedsUpdate={Boolean(macroNeedsUpdate)} onSelectCandidate={(candidateId) => setSelection({ type: "candidate", id: candidateId })} onSetPreference={setPreference} onDiscover={() => startCta("interests", "interest.supplement")} onAddCandidate={addCandidate} onUpdatePlace={updatePlace} onPreviewGoogleMapsLink={previewGoogleMapsLink} onApplyGoogleMapsLink={applyGoogleMapsLink} onRemoveCandidate={removeCandidate} onContinue={async () => gotoStep("detail")} onGoToSkeleton={() => gotoStep("skeleton")} onRetry={retryResolutions} onBeginMapPick={(placeId) => { setMapPickPlaceId(placeId); setFocus("map"); }}/>
