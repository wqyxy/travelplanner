...
import { WorkspaceMapV2, type WorkspaceMapFocusRequest } from "./WorkspaceMapV2";
...
export default function AppWorkflowV3() {
  ...
  const [mapFocusRequest, setMapFocusRequest] = useState<WorkspaceMapFocusRequest | null>(null);
  const [macroPreviewDayId, setMacroPreviewDayId] = useState<string | null>(null);
  const [macroFocusedDayId, setMacroFocusedDayId] = useState<string | null>(null);
  ...
  const gotoStep = (next: WorkflowStepV3) => { setStep(next); setSelection(selectionForWorkflowStepV3(next)); setMapPickPlaceId(null); setMapFocusRequest(null); setMacroPreviewDayId(null); setMacroFocusedDayId(null); setFocus(null); };
  ...
  const loadTrip = async (id: string, resetSelection = true) => {
    ...
    setMacroPreviewDayId(null);
    setMacroFocusedDayId(null);
    setError("");
    try {
