import { ArrowRight, RefreshCw, Route, Sparkles, TriangleAlert } from "lucide-react";
...
export function MacroItineraryPanelV3({
  ...
  const dirtyRoutes = routeStates.filter((state) => state.required && state.dirty).length;
  ...
  return <section className="workspace-requirements-v3">
    ...
    <div className="macro-days-v3">
      {plan.days.map((day) => {
        ...
        return <button type="button" className={`macro-day-v3 ${selectedDayId === day.id ? "active" : ""}`} key={day.id} onClick={() => onSelectDay(day.id)}>
          ...
          <small>{transfer ? `${day.transferMode ?? "未设置交通方式"}${route?.route?.durationMinutes != null ? ` · ${Math.round(route.route.durationMinutes)} 分钟` : route?.dirty ? " · 路线需更新" : " · 路线待定位"}` : "停留日"}</small>
        </button>;
      })}
    </div>
    ...
    <div className="workspace-primary-actions-v3">
      ...
      {dirtyRoutes > 0 && <button className="button" type="button" disabled={busy} onClick={() => void onRecalculateDirty()}><Route size={15}/>更新 {dirtyRoutes} 条城市间路线</button>}
