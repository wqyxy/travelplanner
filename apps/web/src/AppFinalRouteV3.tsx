...
import { downloadFinalRouteCsvV3 } from "./final-route-export-v3";
...
export default function AppFinalRouteV3() {
  ...
  const [hoveredRouteNodeId, setHoveredRouteNodeId] = useState<string | null>(null);
  const [hoveredRoutePlaceId, setHoveredRoutePlaceId] = useState<string | null>(null);
  ...
  const clearRouteTransientState = () => { setHoveredNodeId(null); setHoveredRouteNodeId(null); setHoveredRoutePlaceId(null); setEditingNodeId(null); setMapPickPlaceId(null); setMapPickReturnNodeId(null); setMapFocusRequest(null); };
  const cancelMapPick = () => {
  ...
  const copyRouteNode = async (nodeId: string, targetIndex: number) => {
    ...
    const result = await executeCommands([{
      ...
      index: targetIndex,
      node: { ...source, id: temporaryNodeId, endsDay: false, transportFromPrevious: null },
    }], "无法复制线路地点。");
    return result?.idMappings[temporaryNodeId] ?? null;
  };
  ...
  return <main className={`app-shell app-shell-v3 ${sidebar ? "sidebar-open" : "sidebar-closed"}`}>
    ...
    <section className="main-panel">
      ...
      <div className="main-workspace main-workspace-v3"><div className={`travel-workspace travel-workspace-v3 ${focus ? `focus-${focus}` : ""}`} ref={workspaceElement} style={{ gridTemplateColumns: `${settings.ui.workspaceSplitRatio}fr 10px ${1 - settings.ui.workspaceSplitRatio}fr` }}>
        {workspace ? <FinalRouteMapV3 workspace={workspace} selectedNodeId={selectedNodeId} hoveredNodeId={hoveredNodeId} hoveredRouteNodeId={hoveredRouteNodeId} hoveredRoutePlaceId={hoveredRoutePlaceId} focusRequest={mapFocusRequest} mapPickPlaceId={mapPickPlaceId} fullscreen={focus === "map"} onSelectNode={(nodeId) => { setSection("route"); setSelectedNodeId(nodeId); setMapFocusRequest(null); }} onHoverNode={setHoveredNodeId} onHoverRoute={(nodeId, placeId = null) => { setHoveredRouteNodeId(nodeId); setHoveredRoutePlaceId(placeId); }} onFocusNode={(nodeId) => { setSection("route"); setSelectedNodeId(nodeId); setMapFocusRequest({ nodeId, requestId: ++mapFocusSequence.current }); }} onFocusRoute={(nodeId, placeId = null) => { setSection("route"); setSelectedNodeId(nodeId); setMapFocusRequest({ nodeId, placeId, kind: "route", requestId: ++mapFocusSequence.current }); }} onMapPick={(placeId, latitude, longitude) => void setManualResolution(placeId, latitude, longitude)} onCancelMapPick={cancelMapPick} onFocusHandled={(requestId) => setMapFocusRequest((current) => current?.requestId === requestId ? null : current)} onToggleFullscreen={() => setFocus((current) => current === "map" ? null : "map")}/> : <section className="workspace-map-v2 no-trip"><div className="map-empty-overlay"><MapPinned size={34}/><strong>你的下一段旅程，从这里开始</strong><span>创建旅行、添加地点，在地图上查看完整路线</span></div></section>}
        ...
        <div className={`workspace-side-v3 ${section === "route" && editingNodeId ? "route-editor-open-v4" : ""}`}>{workspace ? <>
          <nav className="final-route-workspace-nav-v3" aria-label="旅行工作区"><button type="button" aria-current={section === "planning" ? "page" : undefined} className={section === "planning" ? "active" : ""} onClick={openPlanning}><span>1</span><b>规划 · 旅行需求</b></button><button type="button" aria-current={section === "route" ? "page" : undefined} className={section === "route" ? "active" : ""} onClick={() => setSection("route")}><span>2</span><b>行程 · 最终线路</b></button></nav>
        ...
        </> : <div className="workspace-empty-v3"><span className="brand-mark">✦</span><h2>选择一趟旅行</h2><p>从左侧选择旅行，或点击“新建旅行”开始规划。</p></div>}</div>
      </div></div>
    </section>
