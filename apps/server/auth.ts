...
export function validatePassword(password: string) { if (password.length < 8) throw new Error("访问密码至少需要 8 个字符。"); if (password.length > 128) throw new Error("访问密码不能超过 128 个字符。"); }
const TTL = 30 * 24 * 60 * 60 * 1000;
export class PersistentSessionStore {
  private revoked = new Map<string, number>();
  constructor(private readonly record: () => PasswordRecord) {}
  ...
  private key() { const hash = this.record().passwordHash; return hash && /^[a-f0-9]{128}$/i.test(hash) ? Buffer.from(hash, "hex") : null; }
