...
export class TravelStore {
  ...
  getMapState(tripId: string): MapState | null { const row = this.db.prepare("SELECT * FROM map_state WHERE trip_id=?").get(tripId) as Row | undefined; if (!row) return null; const map = parse<Record<string, unknown> | null>(row.map_json, null); return { generation: Number(row.generation), resolvedPlaces: parse<ResolvedPlace[]>(row.resolved_places_json, []), map, status: String(row.status) as MapState["status"], warnings: parse<string[]>(row.warnings_json, []), visualComplete: typeof map?.visualComplete === "boolean" ? map.visualComplete : undefined, updatedAt: String(row.updated_at) }; }
