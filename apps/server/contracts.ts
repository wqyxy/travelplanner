...
export const ResolvedPlaceSchema = z.object({ placeId: IdSchema, geoFingerprint: TextSchema.max(1000), provider: TextSchema.max(120), providerPlaceId: z.string().max(200).nullable(), lat: z.number().finite().min(-90).max(90).nullable(), lng: z.number().finite().min(-180).max(180).nullable(), timezone: z.string().max(120).nullable(), resolution: z.enum(["exact", "approximate", "researched", "ignored", "unresolved"]), confidence: z.number().finite().min(0).max(1).nullable(), resolvedAt: z.string().regex(instantPattern).nullable(), sourceUrl: z.string().url().max(2000).nullable().optional(), sourceTitle: z.string().trim().min(1).max(500).nullable().optional(), decisionReason: z.string().trim().min(1).max(700).nullable().optional() }).strict().superRefine((value, context) => {
  const hasLatitude = value.lat !== null; const hasLongitude = value.lng !== null;
  const hasCoordinates = hasLatitude && hasLongitude;
  if (hasLatitude !== hasLongitude) context.addIssue({ code: "custom", path: [hasLatitude ? "lng" : "lat"], message: "纬度和经度必须同时提供或同时为空。" });
  if ((value.resolution === "exact" || value.resolution === "approximate" || value.resolution === "researched") && !hasCoordinates) context.addIssue({ code: "custom", path: ["lat"], message: "已定位地点必须提供坐标。" });
  if ((value.resolution === "ignored" || value.resolution === "unresolved") && (hasLatitude || hasLongitude)) context.addIssue({ code: "custom", path: ["lat"], message: "ignored 或 unresolved 地点不得保存坐标。" });
  if (value.resolution === "researched" && (!value.sourceUrl || !value.sourceTitle || !value.decisionReason)) context.addIssue({ code: "custom", path: ["sourceUrl"], message: "联网研究定位必须保存来源和理由。" });
  if (value.resolution === "ignored" && !value.decisionReason) context.addIssue({ code: "custom", path: ["decisionReason"], message: "忽略地点必须说明原因。" });
});
...
export const MapEdgeSchema = z.object({ id: IdSchema, dayId: IdSchema, fromVisitId: IdSchema, toVisitId: IdSchema, mode: TransportModeSchema, order: z.number().int().min(0), viaIgnoredVisitIds: z.array(IdSchema).max(80).optional(), bridgedTransportMismatch: z.boolean().optional() }).strict(); export type MapEdge = z.infer<typeof MapEdgeSchema>;
export const DerivedMapRouteSchema = z.object({ edgeId: IdSchema, routeKey: z.string().min(1).max(1000), geometry: z.unknown().nullable(), geometrySource: z.enum(["provider", "straight"]).optional(), distanceKm: z.number().finite().nonnegative().nullable().optional(), durationMinutes: z.number().finite().nonnegative().nullable().optional(), status: z.enum(["ready", "attention"]), warning: z.string().max(700).nullable() }).strict(); export type DerivedMapRoute = z.infer<typeof DerivedMapRouteSchema>;
export const DerivedMapSnapshotSchema = z.object({ visits: z.array(MapVisitSchema).max(7200), edges: z.array(MapEdgeSchema).max(7200), routes: z.array(DerivedMapRouteSchema).max(7200), visualComplete: z.boolean().optional().default(false) }).strict(); export type DerivedMapSnapshot = z.infer<typeof DerivedMapSnapshotSchema>;
...
export const CoordinateResearchOutputSchema = z.object({ schemaVersion: z.literal(1), action: z.enum(["use_coordinates", "ignore"]), canonicalName: z.string().trim().min(1).max(300).nullable(), coordinates: z.object({ latitude: z.number().finite().min(-90).max(90), longitude: z.number().finite().min(-180).max(180) }).strict().nullable(), sourceUrl: z.string().url().max(2000).nullable(), sourceTitle: z.string().trim().min(1).max(500).nullable(), reason: TextSchema.max(700) }).strict().superRefine((value, context) => {
  if (value.action === "use_coordinates") {
    if (!value.canonicalName || !value.coordinates || !value.sourceUrl || !value.sourceTitle) context.addIssue({ code: "custom", message: "use_coordinates 必须提供名称、坐标和来源。" });
    else if (new URL(value.sourceUrl).protocol !== "https:") context.addIssue({ code: "custom", path: ["sourceUrl"], message: "坐标来源必须使用 HTTPS。" });
  } else if (value.canonicalName !== null || value.coordinates !== null || value.sourceUrl !== null || value.sourceTitle !== null) context.addIssue({ code: "custom", message: "ignore 不得携带坐标或来源。" });
});
export type CoordinateResearchOutput = z.infer<typeof CoordinateResearchOutputSchema>;
...
export type MapState = { generation: number; resolvedPlaces: ResolvedPlace[]; map: unknown; status: "idle" | "syncing" | "ready" | "attention"; warnings: string[]; visualComplete?: boolean; updatedAt: string };
...
export const CoordinateResearchOutputJsonSchema = strictJson(z.toJSONSchema(CoordinateResearchOutputSchema)) as Record<string, unknown>;
