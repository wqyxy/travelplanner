...
async function api(request: IncomingMessage, response: ServerResponse) {
  ...
  if (method === "PUT" && url.pathname === "/api/settings/ui") { const input = await body(request); const colors = input.mapCategoryColors; if (colors !== undefined && (!colors || typeof colors !== "object" || Array.isArray(colors) || Object.entries(colors).some(([key, value]) => !(key in mapCategoryColorDefaults) || typeof value !== "string" || !/^#[0-9a-f]{6}$/i.test(value)))) return failure(response, 400, "地图分类颜色必须是已知类别的 #RRGGBB 值。"); const ui: AppConfig["ui"] = { ...config.ui, ...(typeof input.workspaceSplitRatio === "number" ? { workspaceSplitRatio: Math.max(.34, Math.min(.66, input.workspaceSplitRatio)) } : {}), ...(input.theme === "light" || input.theme === "dark" ? { theme: input.theme } : {}), ...(typeof input.requirementsPanelOpen === "boolean" ? { requirementsPanelOpen: input.requirementsPanelOpen } : {}), ...(colors ? { mapCategoryColors: { ...config.ui.mapCategoryColors, ...colors } } : {}) }; await save({ ...config, ui }); return json(response, 200, { settings: { ai: config.ai, ui } }); }
